...
import { RoleConfig, roles } from "@/common/roles";

export default function CharacterChat() {
  const [input, setInput] = useState("");
  const [messages, setMessages] = useState<chatMessage[] | null>(null);
  const [currentRole] = useState<RoleConfig>(roles.rin);
    ...
    const fetchWelcomeMessage = async () => {
      ...
      try {
        const res = await fetch("/api/ollamaChat", {
          ...
          body: JSON.stringify({
            role: currentRole.name.toLowerCase(),
            message: `
            Based on the recent conversation:
            ${contextString}
            Write a short and cozy greeting based on user input language.
          `,
          }),
        });

        const data = await res.json();
        setMessages(prev => [...(prev ?? []), { role: "assistant", content: data.content || "(No welcome message)", sender: currentRole.name }]);
      } catch (err) {
        console.error(err);
        setMessages(prev => [...(prev ?? []), { role: "assistant", content: "(Failed to load welcome message)", sender: currentRole.name }]);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const scheduleEncouragement = () => {
      ...
      const prompt = `
      You are ${currentRole.name}, ${currentRole.overview}
      Based on the recent conversation:
      ${contextString}
      Write a short and cozy greeting based on user input language.
      ...
      - Keep it casual and sweet, under 50 words.
      `;
      ...
      encouragementTimeoutRef.current = setTimeout(async () => {
        try {
          const res = await fetch("/api/ollamaChat", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ role: currentRole.name.toLowerCase(), message: prompt }),
          });
          const data = await res.json();
          const content = data.content || "(No encouragement message)";
          setMessages((prev) => [...(prev ?? []), { role: "assistant", content, isAuto: true, sender: currentRole.name }]);
        } catch (err) {
          console.error(err);
        } finally {
  ...
  const sendMessage = async () => {
    ...
    const newMessages = [
      ...(messages ?? []),
      { role: "user", content: input, sender: "You" } // 新增 sender
    ];
    setMessages([...newMessages, { role: "assistant", content: "", sender: currentRole.name }]);
    setInput("");
    setLoading(true);

    const recentMessages = newMessages.slice(-20);
    const contextString = recentMessages.map(m => `${m.sender}: ${m.content}`).join("\n");
    ...
    Write a reply based on user input language},
    keeping it consistent with the role's personality and style.
    `;
    ...
    try {
      ...
      const animate = () => {
        if (i <= content.length) {
          setMessages((prev) => {
            const updated = [...(prev ?? [])];
            updated[updated.length - 1] = { role: "assistant", content: content.slice(0, i), sender: currentRole.name };
            return updated;
          });
          i++;
      ...
      setMessages((prev) => {
        const updated = [...(prev ?? [])];
        updated[updated.length - 1] = { role: "assistant", content: "(An error occurred)", sender: currentRole.name };
        return updated;
      });
    } finally {
      setLoading(false);
        ...
        {(messages ?? []).map((m, i) => (
          <div
            ...
            className={
              m.role === "assistant"
                ? m.isAuto
                  ? "characterchat-auto-message"
                  : ""
                : ""
            }
          >
            <div className="message-role">{m.sender}</div>
            <ReactMarkdown remarkPlugins={[remarkGfm]} rehypePlugins={[rehypeRaw]}>
              {m.content}
            </ReactMarkdown>
          </div>
        ))}
        {loading && <p>{currentRole.name} is thinking…</p>}
